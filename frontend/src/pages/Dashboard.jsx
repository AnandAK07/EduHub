...
import Lectures from '../components/users/Lectures'
        ...
        <>
            <div>
                ...
                <div x-data="{ sidebarOpen: false }" className="flex h-screen bg-gray-200">
                    <div onClick={() => setSideBarOpen(!sidebarOpen)} className={`fixed inset-0 z-20 transition-opacity bg-black opacity-50 lg:hidden ${sidebarOpen ? 'block' : 'hidden'}`}></div>
                    ...
                    {toggle ? <Lectures setSideBarOpen={setSideBarOpen} sidebarOpen={sidebarOpen} /> : <AdminPanel setSideBarOpen={setSideBarOpen} sidebarOpen={sidebarOpen} />}

                </div>
            </div>

            <button onClick={() => { setToggle(!toggle) }}>AdiminToStudents</button>
        </>
    )
}
